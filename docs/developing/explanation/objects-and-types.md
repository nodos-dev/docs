...
This is why the migration guidance from Nodos {{ legacy_version }} says to replace raw `nosObjectId`
values and C resource structs with `nos::ObjectRef` and `nos::TypedObjectRef`. Manual reference
counting across early returns and exception paths leaks, reliably, in proportion to how much code
you write.
