...
the resource to another graphics API or process. Recording from a thread of your own does not make
submission automatic: that work goes out when the thread flushes, which may be long after you
needed it, so ask for the submit explicitly.
...
!!! danger "Two ways to get this wrong"
    ...
    Every event handed back must eventually reach `WaitGpuEvent`, or it leaks — that call is what
    frees the event, and it clears your handle when it does. A `timeoutNs` of `0` polls rather than
    discards: the event is freed only if the GPU has already finished, and on `NOS_RESULT_TIMEOUT`
    you still hold it.
