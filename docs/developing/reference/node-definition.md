...
!!! info "Nodos {{ nodos_version }}"
    `Nodes/*.nosnode`, discovered automatically. Do not list them in the manifest.

!!! warning "Nodos {{ legacy_version }} and earlier"
    `*.nosdef`, listed explicitly in the manifest's `node_definitions` array. A definition not
    listed there is never loaded, and the node silently does not appear.
...
The split is deliberate: `menu_info` is editor presentation, `node` is the class itself. On
{{ legacy_version }} the presentation half lived in the manifest's `associated_nodes` array
instead.
