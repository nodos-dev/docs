...
| **git** | Required. Both installers can fetch it for you. |
...
=== "Windows (PowerShell)"
    ...
    The script checks for `git` and the Visual C++ Redistributable. Git is installed through
    `winget` when it is available; the VC++ runtime comes from Microsoft's official redistributable
    installer.
...
=== "Linux"
    ...
    The script checks for `git` and offers to install it using whichever package manager it
    detects.
