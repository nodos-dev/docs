...
!!! note "What you need"
    ...
    - `git` on your `PATH`. The installer can fetch it for you if it is missing.
...
The installer checks for prerequisites — `git` on both platforms, plus the Microsoft Visual C++
Redistributable on Windows — and offers to install anything missing.
