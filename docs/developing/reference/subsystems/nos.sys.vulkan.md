...
Describes `nos.sys.vulkan` **8.0** on Nodos {{ nodos_version }}. Code written against an earlier
version of this subsystem does not compile against it — see
[Migrate a plugin to {{ nodos_version }}](../../how-to/migrate-1-3-to-1-4.md#6-update-vulkan-subsystem-calls).
