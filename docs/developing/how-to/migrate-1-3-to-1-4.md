...
| Resources | `nosResourceShareInfo` structs you fill in | Reference-counted objects held by `nos::ObjectRef` / `nos::TypedObjectRef` |
...
Resources used to travel as `nosResourceShareInfo` structs that you filled in and passed by
pointer. On {{ nodos_version }} they are engine objects with reference counting, so hold them as
`nos::ObjectRef` / `nos::TypedObjectRef` instead. Those are RAII wrappers over that counting, and
they remove a class of leak that was easy to hit by hand. See
[Objects and the type system](../explanation/objects-and-types.md).

## 6. Update Vulkan subsystem calls

If your plugin uses `nos.sys.vulkan`, the API changed shape along with everything else:

| | Before ({{ legacy_version }}) | After ({{ nodos_version }}) |
|---|---|---|
| Include path | `<nosVulkanSubsystem/nosVulkanSubsystem.h>` | `<nosSysVulkan/nosVulkanSubsystem.h>` |
| Getting the API | `nosEngine.RequestSubsystem` by hand | `NOS_DEPENDENCY(NOS_VULKAN)` |
| Resources | `nosResourceShareInfo*` | Object handles — `nosResourceObject`, `nosTextureObject` |
| Command begin/end | `Begin(name, outCmd)`, `End(cmd, forceSubmit)` | `nosCmdBeginParams` / `nosCmdEndParams` |
| GPU event on submit | Separate `End2(cmd, forceSubmit, outEvent)` | `nosCmdEndParams::OutGPUEventHandle` |

The current shape of every call is in [`nos.sys.vulkan`](../reference/subsystems/nos.sys.vulkan.md).
...
- [ ] `nosResourceShareInfo` structs replaced with `nos::ObjectRef` handles
- [ ] `nos.sys.vulkan` calls moved to object handles and the `nosCmd*Params` structs
