...
`-v, --version <version>`
:   Release version. Default `1.3`. Pass `{{ nodos_version }}` for the current line.
...
`-n, --nodos-version <VERSION>`
:   Nodos version to target. Determines whether `Nodes/*.nosnode` is used
    ({{ nodos_version }}+) or the manifest's `node_definitions` array ({{ legacy_version }}).

!!! danger "Pass `-n {{ nodos_version }}` explicitly on the current line"
    The documented fallback ("the latest version will be used") applies only when *writing* a
    definition. The lookup that checks whether the class already exists does not use it, so without
    `-n {{ nodos_version }}` this command cannot see existing `Nodes/*.nosnode` files:

    - Adding a class that already exists **overwrites its definition and discards its pins**, with
      no error.
    - `--remove` fails with `Node class ... not found`.

    Both behave correctly with `-n {{ nodos_version }}`. See
    [Known issue: node definition discovery](../../developing/how-to/add-nodes-and-pins.md#known-issue-node-definition-discovery).
...
!!! danger "Unusable on Nodos {{ nodos_version }} — no workaround"
    This command registers no `--nodos-version` argument, so it always resolves node classes as
    though targeting {{ legacy_version }}: through a manifest's `node_definitions` array only. It
    never scans `Nodes/*.nosnode`, and so reports `Node class ... not found` for every node created
    by `nodos node` on {{ nodos_version }}.

    Unlike `nodos node`, there is no flag that changes this. Edit the `.nosnode` file directly —
    see [Add nodes and pins](../../developing/how-to/add-nodes-and-pins.md).

    It works normally on {{ legacy_version }}, where definitions are listed in the manifest.
