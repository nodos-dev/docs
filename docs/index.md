...
Nodos {{ legacy_version }} is still supported and differs in ways that matter to plugin authors —
manifest file extensions, node definition discovery, and the CMake workflow all changed. Those
differences are called out inline, and collected in
[Migrate a plugin to {{ nodos_version }}](developing/how-to/migrate-1-3-to-1-4.md) and
