...
This tutorial targets Nodos {{ nodos_version }}. If you are on {{ legacy_version }}, the manifest
and build steps differ substantially; see
...
```shell
nodos node mycorp.myplugin PrintFloat -n {{ nodos_version }} \
...
!!! danger "Pass `-n {{ nodos_version }}` every time"
    Without it the command assumes Nodos {{ legacy_version }} and cannot see existing
    `Nodes/*.nosnode` files. Run it a second time for a class that already exists and it overwrites
    the definition, **silently discarding every pin you have added to it**.

    Get into the habit now — you will re-run this command, and the failure is completely silent.
    Details in [Add nodes and pins](../how-to/add-nodes-and-pins.md#known-issue-node-definition-discovery).
...
!!! warning "`nodos pin` cannot be used here"
    There is also a `nodos pin` command for adding pins, but as of nosman 0.17 it does not work on
    Nodos {{ nodos_version }} definitions — it only finds node classes listed in a manifest's
    `node_definitions` array, the {{ legacy_version }} layout, and never scans `Nodes/*.nosnode`.
    Unlike `nodos node` it accepts no `-n` argument, so there is no way to tell it otherwise.

    Editing the `.nosnode` file directly, as above, is the way to add pins. See
    [Add nodes and pins](../how-to/add-nodes-and-pins.md#known-issue-node-definition-discovery).

!!! info "Nodes are discovered automatically"
    On {{ nodos_version }}, every `.nosnode` under `Nodes/` is picked up. You do not list them in
    the manifest. On {{ legacy_version }} you did, and forgetting to was a common reason for a node
