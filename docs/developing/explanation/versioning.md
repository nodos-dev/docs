...
**Engine version** — `{{ nodos_version }}.0`. The Nodos release line. It determines the manifest
and node definition formats, and the build workflow.
...
Earlier lines are still supported and still shipping, and `nodos get` still defaults to one of
them. Ask for the current line explicitly:
