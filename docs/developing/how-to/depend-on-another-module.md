...
pointer is valid. But when you request a subsystem by hand with `nosEngine.RequestSubsystem` — for
an optional dependency, say — the request can fail because the requested version is not installed.
