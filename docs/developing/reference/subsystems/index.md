...
There is no structural difference between the two: both use a `.nosplugin` manifest, and
`nodos create` produces the same scaffold. The distinction is what the module is *for*.
...
2. Import it with `NOS_DEPENDENCY(...)`.
